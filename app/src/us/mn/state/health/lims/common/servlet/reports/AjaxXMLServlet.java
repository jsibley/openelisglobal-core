...
import us.mn.state.health.lims.common.provider.reports.BasePrintProvider;
import us.mn.state.health.lims.common.provider.reports.PrintProviderFactory;
...
public class AjaxXMLServlet extends AjaxServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String repProvider = request.getParameter("provider");
		BasePrintProvider provider = (BasePrintProvider) PrintProviderFactory
				.getInstance().getPrintProvider(repProvider);
		provider.setServlet(this);
		try {
			provider.processRequest(request, response);
		} catch (LIMSPrintException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (LIMSInvalidPrinterException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
